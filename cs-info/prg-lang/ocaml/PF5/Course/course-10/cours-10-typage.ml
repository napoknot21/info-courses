...
(** Quelques mots de plus sur le type result. Ce type, qui a deux
    paramètres de types, est défini dans la bibliothèque standard OCaml,
    et ressemble à ceci : *)
...
(** Par contre on ne généralise pas les variables d'un let quand il
    ...
    dites faibles ou existentielles, sont notées '_weak1 '_weak2 etc. *)

let f = List.map (fun x -> x)  (* '_weak1 list -> '_weak1 list *)
